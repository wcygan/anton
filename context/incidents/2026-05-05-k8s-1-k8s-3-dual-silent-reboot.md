...
2. **Two-node simultaneous reboot, k8s-2 held** — *inverse* of every prior incident, where k8s-2 was always the affected node.
3. **BIOS version not predictive**: k8s-1 is on **1.26** (untouched), k8s-3 is on **1.27** (post-flash 2026-05-04), k8s-2 is on **1.27** (post-flash 2026-05-04). The two rebooted nodes span both BIOS versions; the held node shares a BIOS version with one of the rebooted nodes.
