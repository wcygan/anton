...
> **2026-05-05 update — "k8s-2-only" framing falsified.** A cross-node silent-reboot event hit **k8s-3 at 19:07:13Z (verified via live `/proc/uptime`, boot ID `e55639d4…`)** then **k8s-1 at ~19:33Z (postmortem estimate, ~26 min later)** while **k8s-2 held cleanly** — the inverse of every prior incident in this README. The event was originally written up as "simultaneous within ~28 s" but verification on the live cluster showed that was wrong by ~26 min for k8s-3; the actual signature is a **sequential cascade**. Plan 0009 closed Abandoned and superseded by [plan 0013](../../plans/0013-cluster-wide-silent-reboot-localization.md), which picks up under the cluster-wide / sequential-cascade hypothesis space. See [`evidence-2026-05-05-dual-silent-reboot.md`](evidence-2026-05-05-dual-silent-reboot.md) (note correction banner), [`../../postmortems/2026-05-05-k8s-1-k8s-3-dual-silent-reboot.md`](../../postmortems/2026-05-05-k8s-1-k8s-3-dual-silent-reboot.md), and [`../../incidents/2026-05-05-k8s-1-k8s-3-dual-silent-reboot.md`](../../incidents/2026-05-05-k8s-1-k8s-3-dual-silent-reboot.md). **The "Current status" paragraph and "Five live candidates" list below describe the pre-2026-05-05 state and are preserved for historical context — the live ranking now lives at the bottom of this section under "Re-ranking 2026-05-05".**
...
- 2026-04-23 20:45Z: **k8s-3 was NOT a silent reboot** — operator-initiated `talosctl reboot` at 20:44:37Z as Harbor plan 0006 acceptance test; confirmed 2026-04-24 via plan 0006 Log line 115. k8s-3 has never silently rebooted during the investigation. **Note 2026-05-05:** "never silently rebooted" was true through 2026-05-04 but is no longer true — see entry below.
- 2026-05-05 19:07:13Z: **k8s-3 silently rebooted (verified via live `/proc/uptime`, boot ID `e55639d4…`)** — first verified silent reboot of k8s-3 during the investigation
- 2026-05-05 ~19:33Z: **k8s-1 silently rebooted (~26 min after k8s-3, postmortem estimate; not independently verifiable since the 20:29:07Z graceful reboot overwrote on-node evidence)** — first silent reboot of k8s-1 in 190 days of cluster age; k8s-2 held throughout. Originally framed as "simultaneous with k8s-3 within ~28 s"; corrected when live `/proc/uptime` showed k8s-3's actual boot was 26 min earlier than the recovery-time dmesg estimate
- k8s-1: was 0 reboots / 7d+ continuous uptime through 2026-05-04; first silent reboot 2026-05-05 ~19:33Z

**Silent-reboot total: 4 events** — 2 on k8s-2 (04-21, 04-23), 1 on k8s-3 (2026-05-05 19:07:13Z), 1 on k8s-1 (2026-05-05 ~19:33Z). The 04-23 k8s-3 event was operator-initiated and is excluded; early analysis mis-identified it as a cascade. The 2026-05-05 cross-node event is the first verified organic silent reboot of k8s-1 or k8s-3.
...
### Re-ranking 2026-05-05 — single-unit hypothesis falsified, cascade and cluster-wide candidates move to the top

The 2026-05-05 cross-node sequential reboot (k8s-3 19:07:13Z, k8s-1 ~19:33Z, ~26 min apart, k8s-2 held) inverts the "k8s-2 single-unit defect" reading that had been #1 above. Provisional new ranking pending plan 0013 Phase 2 gap analysis on k8s-1's 19:07Z → 19:33Z window:

1. **Sequential cascade triggered by one node's failure stressing another** (NEW; leading) — k8s-3's 19:07Z silent reboot may have triggered a 26-min escalation on k8s-1 (etcd churn / leader transfer, kubelet/kube-apiserver recovery, Cilium endpoint or identity churn, network re-convergence, conntrack pressure). Plan 0013 Phase 2 gap analysis is the cheap discriminator; Phase 3's controlled `talosctl reboot` of k8s-3 with kernel sink live is the higher-cost reproduction test
2. **Cluster-wide kernel / network mechanism** (was prior #2; strengthened by the cross-node fingerprint) — Talos kernel CPU-wedge produces exactly the silent-reset signature, and a mechanism that propagates between nodes via the data plane fits the 26-min gap if the propagation step is bounded by some queue / timeout / retry limit
3. **kube-apiserver memory pressure** (was prior #3; orthogonal) — observed pre-2026-05-05 on k8s-2 only; status unchanged
4. **CNI / Multus / VXLAN cascade** (was prior #4; in play) — the cross-node spread keeps this candidate alive; plan 0013 Phase 3 includes a controlled VXLAN-storage-overlay stress test with the kernel sink live
5. **Single-unit defect on k8s-2's chassis** (was prior #1; **falsified as the dominant cause**) — k8s-2 was the node that *held* on 2026-05-05 while two others rebooted. DIMM-swap and BIOS work on k8s-2 may still explain k8s-2's own past instability, but cannot be the cluster-wide root cause. Demoted from plan-critical to "do during the next physical visit if convenient" in plan 0013 Phase 3
6. **Power / electrical event affecting two of three chassis** (added 2026-05-05, then demoted same day) — the verified 26-min gap between k8s-3 and k8s-1 weakens the shared-power-coincidence hypothesis materially (a power event would have hit both nearly simultaneously). Plan 0013 acceptance criterion 3 demoted from "must-be-resolved gate" to "audit-and-record" static reference

The full backlog of discriminating tests now lives in plan 0013 Phase 2 (cheap retrospective) and Phase 3 (controlled reproductions). The fire-test Phase 1 verification of the now-restored Vector kernel sink + per-node reboot alerts + Unknown-pod alerts also lives there.
...
| [`evidence-2026-04-29-stress-stage3.md`](evidence-2026-04-29-stress-stage3.md) | Stage 3 multicore CPU + memory-bandwidth stress-ng pass on k8s-2 (20 cpu + 8 vm stressors, 90 min, clean, 0 verify failures across ~228 M bogo-ops) — closes the userland-reachable load/bandwidth/thermal subspace; only Stage 4 (boot-time memtest86+ / DIMM swap) and a `performance`-governor re-run remain on the diagnostic surface |
| [`2026-04-29-stages-1-3-synthesis.md`](2026-04-29-stages-1-3-synthesis.md) | Synthesis after Stages 1+2+3 plus 19.6 h post-reboot uptime: hypothesis-ranking deltas, what the Vector kernel sink confirms (zero fault precursors at max load), tests-by-signal-per-cost table (A=Stage 4 / B=sysrq-c netconsole validation / C=long-duration moderate stress / D-F lower priority), and the recommended sequence |
| [`evidence-2026-04-29-stage35-stress.md`](evidence-2026-04-29-stage35-stress.md) | Stage 3.5 long-duration stress-ng pass on k8s-2 (4 cpu + 2 vm + 4 switch + 2 mmap, 4 hours, clean) — node survived full test then silently rebooted ~5h later at idle (R8), confirming stress tests are testing the wrong axis |
| [`evidence-2026-04-30-web-research.md`](evidence-2026-04-30-web-research.md) | Web research: MS-01 + BIOS 1.22 instability reports, Intel Raptor Lake Vmin shift "passes stress / fails at idle" pattern, Talos watchdog (iTCO_wdt) unfed reboot risk, NVMe/ASPM power state silent reboot, DDR5 training issues. Updated hypothesis ranking and 40+ external references |
| [`evidence-2026-05-04-bios-1.27-flash.md`](evidence-2026-05-04-bios-1.27-flash.md) | Physical intervention 2026-05-04: BIOS 1.22 → 1.27 flash on k8s-2 and k8s-3, k8s-1 left on 1.26 as anchor. Pre/post boot IDs, Minisforum boot-menu and post-flash USB-shell quirks, R9 silent reboot detected at session start, post-flash flag (k8s-2 boot ID changed again during break — possibly operator-initiated, possibly silent). DIMM swap deferred. 48-72 h watch window starts now |
| [`evidence-2026-05-05-dual-silent-reboot.md`](evidence-2026-05-05-dual-silent-reboot.md) | 2026-05-05 cross-node silent reboot: k8s-3 19:07:13Z (verified), k8s-1 ~19:33Z (estimated, ~26 min later), k8s-2 held. Per-node BIOS/boot-ID matrix, forensic posture, hypothesis re-cut. Includes correction banner — original "simultaneous within ~28 s" framing was wrong; actual signature is sequential cascade |
| Rollout / execution state | [`../../plans/0007-k8s-2-remote-diagnostic-rollout.md`](../../plans/0007-k8s-2-remote-diagnostic-rollout.md) for diagnostic rollout/teardown; [`../../plans/0010-stage-k8s-2-software-rejoin.md`](../../plans/0010-stage-k8s-2-software-rejoin.md) for staged k8s-2 rejoin; [`../../plans/0013-cluster-wide-silent-reboot-localization.md`](../../plans/0013-cluster-wide-silent-reboot-localization.md) (current owner of root-cause work, supersedes plan 0009) |
