...
- 2026-05-04 (post-close cleanup — rejoin gating physically unwound): Plan 0010 closed by abort 2026-04-28 with the `anton.io/rejoin=k8s-2` label and matching `NoSchedule` taint left intentionally in `talos/talconfig.yaml` so the cordon survived cluster reboots. With the BIOS 1.27 flash now landed (plan 0009 Phase 5, 2026-05-04) and k8s-2 stable >14 h on boot ID `28688229-2d15-4216-b028-8b558a5530fe`, both label and taint were dropped from `talconfig.yaml` and `talosctl apply-config --mode=auto` rolled the change to k8s-2 (no reboot, etcd 3/3, boot ID unchanged). Longhorn `allowScheduling: true` re-enabled on the `nodes.longhorn.io/k8s-2` CR. The four staged-rejoin diagnostic apps (`playground/k8s-{1-rejoin-control,2-bounded-stress,2-rejoin-smoke,rejoin-mirrored-load}`) were also deleted in the same commit (`4037deec`). k8s-2 is now a normal cluster member again — full read/write scheduling, full Longhorn replica eligibility, no `anton.io/rejoin` taint anywhere. Plan 0010 stays Closed; this entry is the durable record that the Phase 1 scheduling guardrails it added are no longer in the rendered Talos config.
