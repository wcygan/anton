...
- [x] Update `.claude/skills/observability-integrate/` with a reference to the established dashboard shape. Actual shape (updated from plan's original language): header stats + **always-expanded** subsystem rows (operator preference superseded the ADR's "top-2 default-expanded"), `uid`-stable for URL continuity, debug rows pair a 0=healthy stat strip with a detail table. Landed in `SKILL.md` ("Anton dashboard shape" section) and `references/grafana-dashboards.md` ("Anton patterns" section — documents the `"0 — healthy"` stat tile recipe, `or vector(0)` fallback for count queries, unified multi-source table via `or` + `label_replace`, and the three PromQL substitutions that actually work on anton: Cilium datapath counters instead of Hubble flows, `longhorn_replica_state == 1` instead of `longhorn_volume_robustness` for replica count, `flux_resource_info{ready="True"}` instead of `gotk_reconcile_condition`).
...
- 2026-04-17: Phase 1d complete (out of sequence — landed before the Phase 1c operator walk, at operator request to capture fresh patterns while they were loaded in context). Edits landed in `.claude/skills/observability-integrate/`: `SKILL.md` gained an "Anton dashboard shape" section under the dashboard workflow, and `references/grafana-dashboards.md` gained an "Anton patterns" section documenting (a) the `"0 — healthy"` stat tile recipe (`colorMode: "background"` + value mapping + `graphMode: "none"`), (b) the `or vector(0)` fallback for additive count queries, (c) the unified multi-source table pattern (`or` chain with `label_replace` adding a discriminator column), and (d) three PromQL substitutions that are already load-bearing on anton and should not be re-derived (Cilium datapath counters instead of Hubble flows, `longhorn_replica_state == 1` for Longhorn replica count, `flux_resource_info{ready="True"}` for Flux rollups). Future workload dashboards should extend this shape rather than invent their own.
