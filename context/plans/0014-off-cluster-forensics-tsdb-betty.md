...
### Phase 3.5: Etcd metrics scrape (2026-05-06)

- [x] **Add etcd peer-metrics scrape job to `/home/wcygan/vmsingle-config/scrape.yml`** — `anton-etcd` job, port 2381, tailnet IPs, relabeled to anton's in-cluster instance convention. Reloaded vmsingle via `POST /-/reload`. Verified 132 etcd_* metrics flowing; targets `health=up` for all three nodes. Useful for retrospective queries that need `etcd_server_*` metrics — particularly the iter-#29 silent-reboot precursor expression which joins `node_network_carrier` (already scraped via node-exporter) with `etcd_server_proposals_failed_total`.
...
- 2026-05-06: **Added etcd metrics scrape job to scrape.yml.** Iter-#29's silent-reboot precursor expression joins `node_network_carrier` (already scraped) with `etcd_server_proposals_failed_total` (only on etcd's `:2381/metrics`). Adding etcd makes the precursor expression evaluable retrospectively against betty's TSDB. iter-#4's "≥48h retention" concern was already addressed by the original `-retentionPeriod=30d` flag set 2026-05-05.
- 2026-05-06: **Briefly tried + rolled back vmalert backstop alerting.** Authored a vmalert rule file (`SilentRebootPrecursor`, `AntonEtcdQuorumDegraded`, `AntonNodeUnreachableFromBetty`) and launched vmalert pointing at ntfy.sh public. Ran for ~10 minutes, verified rules loaded and evaluated cleanly against current data (3/3 inactive — cluster quiet). **Operator chose not to keep it**: the operator can detect anton anomalies through other channels (anton's own Alertmanager → ntfy per ADR 0026, manual checks, ssh) and would rather query betty's TSDB retrospectively when an event happens. Backstop alerting adds a notifier-routing dependency (ntfy.sh public vs self-hosted) that doesn't pull its weight for a homelab whose primary alert path already exists. Tore down vmalert container + image, deleted `alerts.yml` + `.ntfy-topic`. Kept the etcd scrape addition (still useful for retrospective queries). Updated `README.md` to reflect the simpler "passive scraper, query on demand" operating model.
