...
- [ ] **Add a health probe for instance-manager NAD attach.** The 2026-05-06/07 harbor-postgres incident proved Flux ordering + Longhorn + Multus DaemonSet readiness are all insufficient — Multus' CNI-add can fail on a single pod silently and Longhorn keeps reporting that IM `1/1 Running`. Detection options, in order of effort: (a) **PrometheusRule** on a metric exposed by Multus or scraped from `kubectl get pod -o json` that compares `k8s.v1.cni.cncf.io/networks` request to `k8s.v1.cni.cncf.io/network-status` actual — alert at `severity: critical` because this silently breaks every cross-node Longhorn attach on the affected node; (b) **CronJob** that checks each `instance-manager-*` pod every 5 min and emits a metric if `lhnet1` is not present in the actual interface list; (c) **liveness/readiness probe** patched onto the instance-manager Pod template that fails if `lhnet1` is missing — most surgical but requires Longhorn chart customization. Hand the chosen option off to `observability-integrate`. Rule should also fire on `multus-upstream` / `whereabouts` pod restart counts — repeat restarts indicate the underlying CNI plugin is the broken link.
