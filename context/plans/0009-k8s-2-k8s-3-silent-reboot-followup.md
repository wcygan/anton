...
related-adrs: [0005, 0017, 0020, 0021]
...
- [ ] ADR 0020 superseded by a new ADR raising cilium-agent memory limit to ≥ 3584 MiB and setting `clean-cilium-state: true`, citing cilium/cilium #42007 and #37935
...
- [ ] Commit + push; watch Flux HelmRelease reconcile; watch cilium DaemonSet rollout (should be ~60s per pod, no node blip)
- [ ] Post-rollout check: all three cilium pods show new limit via `kubectl -n kube-system get pod -l k8s-app=cilium -o jsonpath='{range .items[*]}{.metadata.name} {.spec.containers[?(@.name=="cilium-agent")].resources.limits.memory}{"\n"}{end}'`. Record baseline WSS/Cache at T+5 min and T+60 min
- [ ] Log ADR 0020's "durable-fix" claim as partially retracted — the memory bump extended stability but did not eliminate the mechanism. The new ADR should acknowledge this explicitly
