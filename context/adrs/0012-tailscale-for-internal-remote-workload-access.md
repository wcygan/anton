...
> **Amended 2026-04-17** — in-place correction of factual errors in the body discovered during the first Grafana rollout. Decision direction is unchanged (Tailscale for internal remote; Cloudflare tunnel reserved for public). The mechanism described was wrong: the operator's Service-annotation recipe is a TCP passthrough with no TLS in the browser, and certs on the Ingress recipe are Let's Encrypt (via MagicDNS), not "Tailscale's CA". A throughput caveat (tailscale/tailscale#16198) has been added to Consequences.
...
- [x] Expose Grafana via Tailscale `Ingress` (first concrete application; done 2026-04-16).
- [x] Rewrite `expose-service/references/tailscale.md` to describe the Ingress + Service-annotation recipes and remove the blanket prohibition (done 2026-04-16).
- [x] Extend the gateway-choice matrix in `expose-service/SKILL.md` with a row for Tailscale (done 2026-04-16).
