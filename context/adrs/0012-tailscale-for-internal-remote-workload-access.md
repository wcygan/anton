...
- [x] Validate Pattern 3 (`ingressClassName: tailscale`) on a low-risk workload before retrying Grafana — validated 2026-04-17 on the Hubble UI: post-fix first-paint over the tailnet was qualitatively indistinguishable from LAN. Next action authorised: retry Grafana with a scoped Ingress manifest (values-only), no bundled changes.
