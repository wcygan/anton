...
- [ ] **File upstream issue / PR on `controlplaneio-fluxcd/charts/flux-operator`** to add port 8081 to the `web` NetworkPolicy ingress rules. Once merged and a chart bump lands in this repo, drop the `web.networkPolicy.create: false` override on the flux-operator HelmRelease and revert to chart defaults.
- [ ] **Inline comment in `kubernetes/apps/flux-system/flux-operator/app/helmrelease.yaml`** updated to point at the upstream tracker once it exists, so the override has a clear retirement signal.
