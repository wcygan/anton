...
> ⚠️ **Throughput caveat (ADR 0012 amendment, 2026-04-17).** The operator defaults Ingress-resource proxies to **userspace netstack** (`TS_USERSPACE=true`), which caps per-proxy throughput near 100 Kbps; upstream bug [tailscale/tailscale#16198](https://github.com/tailscale/tailscale/issues/16198) also hits kernel-mode proxies on v1.84+ via oversized TCP segments past the 1280 MTU. Effect: SPA loads over the tailnet are slow (Grafana measured ~12 KB/s from a macOS peer); small API calls feel fine. Anton deploys a `ProxyClass` named `kernel` at `kubernetes/apps/observability/kube-prometheus-stack/app/proxyclass.yaml` (bound via `tailscale.com/proxy-class: kernel` annotation on the Grafana Ingress) that switches to kernel mode + privileged TUN — **prerequisite** for the `ethtool -k tailscale0 tso off gso off` workaround but does not itself fix throughput (ProxyClass has no sidecar or postStart hook, and `ethtool` is not in the Alpine proxy image). Until upstream fix (milestone 1.98.x) lands: prefer `envoy-internal` on LAN; use Tailscale for off-LAN admin access and accept the slowness. Do not copy the ProxyClass blindly onto other workloads — it only helps if a compatible ethtool workaround exists.
