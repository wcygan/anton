...
> ⚠️ **MTU caveat (ADR 0012 amendment 2026-04-17).** In Cilium kube-proxy-replacement mode Cilium auto-detects every interface matching its default device pattern, including `tailscale0`, and propagates the tailnet interface's 1280 MTU to the rest of the datapath — which craters throughput for anything traversing Cilium to or from the Tailscale proxy ([tailscale/tailscale#12393](https://github.com/tailscale/tailscale/issues/12393), [Tailscale operator × Cilium docs](https://tailscale.com/docs/features/kubernetes-operator#cilium-in-kube-proxy-replacement-mode)). Anton fixes this with a one-line Cilium values change: `devices: enp+` so only physical NICs are auto-detected. If you change the Cilium `devices:` pattern in the future, verify `cilium-dbg status --all-addresses` on each node shows only physical NICs (no `tailscale0`) before relying on tailnet throughput. LAN access through `envoy-internal` is unaffected either way.
