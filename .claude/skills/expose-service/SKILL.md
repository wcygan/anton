...
2. **Author the resource(s):**
   ...
   - **Tailscale path** — two recipes, pick by whether you need HTTPS in the browser. HTTP admin UIs (Grafana, Longhorn, etc.) use the **Ingress** recipe (`ingressClassName: tailscale`, `tls.hosts: [<slug>]`) to get a MagicDNS Let's Encrypt cert. Non-HTTP or raw-TCP workloads use the **Service annotation** recipe (`tailscale.com/expose` + `tailscale.com/hostname`). Full recipes: `references/tailscale.md`.
