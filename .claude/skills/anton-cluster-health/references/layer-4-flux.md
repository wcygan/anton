...
**Verify the Flux-side key**:

```sh
kubectl -n flux-system get secret sops-age -o jsonpath='{.data.age\.agekey}' | base64 -d | head -1
```

That public-key prefix must appear in `.sops.yaml` recipients. If not, the repo was re-keyed without updating the cluster Secret.
...
```sh
# Verify the Secret exists and has keys
kubectl -n flux-system get secret cluster-secrets -o jsonpath='{.data}' | base64 -d 2>/dev/null || \
