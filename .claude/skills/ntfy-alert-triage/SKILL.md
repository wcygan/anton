...
**Ground-truth check before reacting**: a fresh-looking alert can be a metric/series artifact, not a real event. For `NodeUnexpectedReboot` specifically, check `/proc/uptime` directly via `talosctl`; cross-reference with the cluster-triage agent memory at `.claude/agent-memory/cluster-triage/reference_reboot_alert_disambiguation.md`.
...
The webhook URL — including the secret topic — lives in the `ntfy-topic` Secret in `observability`. **Never echo the URL to stdout or write it to a file** (CLAUDE.md hard rule). Pipe it directly:
...
## Hard rules (carry-overs from CLAUDE.md)
