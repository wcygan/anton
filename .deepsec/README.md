...
Currently configured project: `anton` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/anton/SETUP.md` to fill in
   `data/anton/INFO.md`.
...
`--project-id anton` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/anton/`.
