...
Before committing, verify no plaintext `*.sops.*` files: `find . -name '*.sops.*' -not -path './.private/*' -exec sops filestatus {} \;` — all must report `encrypted`. Full task reference: `.taskfiles/CLAUDE.md`.
...
- **Never** run destructive operations without explicit user confirmation: `task talos:reset`, `talosctl reset`, `kubectl delete namespace`.
...
1. Edit the manifest directly under `kubernetes/`, or `talos/talconfig.yaml` / `talos/talenv.yaml` for Talos changes.
2. For new SOPS files: author plaintext, then `sops -e -i <file>`. For existing SOPS files: edit with `sops <file>` (transparent round-trip). Verify with `sops filestatus <file>`.
3. For Talos edits, run `task talos:generate-config` to regenerate `talos/clusterconfig/`.
4. Commit and push. Flux auto-syncs, or run `task reconcile` to force.
5. For Talos or node-level changes, hand off to the `talos-operator` subagent.
